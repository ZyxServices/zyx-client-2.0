 ...
 * @since 2016/11/4
 */
public class FBDTStrategy extends PointParamStrategy {
    @Override
    public UserPointParam buildPointParam(Integer userId, Long pointCount, String detailMsg) {
        ...
        userPointParam.setPointCount(null == pointCount ? PointConstants.POINT_COUNT_FBDT : pointCount);
        userPointParam.setDetailTable(PointConstants.TABLE_PANYAN);
        userPointParam.setDetailType(PointConstants.DETAIL_TYPE_FBDT);
        userPointParam.setDetailMsg(detailMsg + PointConstants.DETAIL_TYPE_MSG_FBDT);
